...
impl MemorySet {
    ...
    pub fn new_kernel() -> Self {
        ...
        println!("uuuuuuuuuuuuuuuuuuuuuuuuuuuuu");
        let k=memory_set.page_table.return_root();
        println!("{}",k.0);
    ...
    //////////////////////////////////////////////////////////注释注释
    pub fn print_all_page_table_info(&self){
        let mut i =0;
        for area in &self.areas {
            // 访问每个区域的 PageTable
            let page_table = &self.page_table;
            println!("Page Table for Memory Area:");
            page_table.print_page_table_info();
...
pub fn print_kernel_space_page_table() {
    let kernel_space = KERNEL_SPACE.lock(); 
    //let root_page_table = kernel_space.page_table;
    kernel_space.print_areas();
    println!("111111111111111111111111");
    kernel_space.page_table.print_HHH();
    println!("222222222222222222222222");
    kernel_space.print_all_page_table_info();
    //println!("Printing Kernel Space Page Table:");
    // let root=kernel_space.page_table.return_root();
    //let k=kernel_space.page_table.traverse_page_table();
    //println!("{}",k);
}