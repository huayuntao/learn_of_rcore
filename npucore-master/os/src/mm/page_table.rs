...
impl PageTableEntry {
    ...
    pub fn is_page_table(&self)-> bool{
        (self.bits & 0b1) == 1 && (self.bits & 0b1110) == 0
    }
}
...
impl PageTable {
    ...
    //////////////////////////////////////////////////////////////注释注释
    pub fn return_root(&self)-> PhysPageNum{
        self.root_ppn
    }
    
    /////////////////////////////////////////////////////////////////////注释注释
    // pub fn traverse_page_table(&self, level: usize, ppn: PhysPageNum) {
    //     let k=ppn.0;
    //     let l:VirtPageNum=VirtPageNum(k);
    //     let m=self.find_pte(l);
    //     for (index, entry) in m.iter().enumerate() {
    //         println!("Level {}: Entry {} - ", level, index);
            
    //         // 如果页表项是页表（非页帧），继续递归遍历下一级页表
    //         if entry.is_page_table() {
    //             self.traverse_page_table(level + 1, entry.ppn());
                
    //         }
    //     }
    // }
    pub fn recursive_traverse_page_table(&self, ppn: PhysPageNum, level: usize,total_entries:&mut usize){
        // 获取当前页表项
        let pte_array = ppn.get_pte_array();
        // 遍历当前页表项
        for (idx, pte) in pte_array.iter().enumerate() {
             if pte.output_ppn() != 0  {
                if level==0 {print!("..  ");}
                if level==1 {print!("..  ..  ");}
                if level==2 {print!("..  ..  ..  ");}
                let z=pte.output_ppn();
            println!("Level {}: Entry {}: pte:{:x}    pa{:x}", level, idx, pte.output(),PhysAddr::from(z).0);
            *total_entries += 1;
             }
            // 如果页表项是页表（非页帧），递归遍历下一级页表
            if pte.is_page_table() {
                self.recursive_traverse_page_table(pte.ppn(), level + 1,total_entries);
            }
        }
    }
    ...
    pub fn traverse_page_table(&self) -> usize {
        // 使用一个计数器来统计输出的总数
        let mut total_entries = 0;
        
        // 从根页表开始递归遍历
        self.recursive_traverse_page_table(self.root_ppn,0,&mut total_entries);
    ...
    ///try to print the message of kernel
    pub fn print_page_table_info(&self) {
        for (index, frame) in self.frames.iter().enumerate() 
        {
            println!("Page Table Entry {} - PPN: {:?}", index, frame.ppn);
        }
    }
    ...
    //////////////////////////////////////////////////////注释注释
    pub fn print_HHH(&self){
        println!("YYYYYYYYYYHHHHHHHHHHH");
    }
    ///////////////////////////////////////////////////////////
}
